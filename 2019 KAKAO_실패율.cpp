...
{
    if(p1.second==p2.second) return p1.first<p2.first; //실패율이 같으면 스테이지가 낮은게 우선 
    return p1.second>p2.second; //실패율이 다르면 실패율 높은 게 우선
}
vector<int> solution(int N, vector<int> stages) {
    ...
    for(int i=1;i<=N;i++)
    {
        ...
        for(const int& stage:stages)
        {
            ...
            //즉 깬 유저 인원을 늘려준다.
            if(stage>=curr_stage) clear_users++;
            //같으면 해당 유저가 도전중인 스테이지가 현재스테이지와 같으면 해당유저는 지금 
            //이 스테이지를 도전중인 유저이다.
            if(stage==curr_stage) progress_users++;
        }
        //해당 스테이지 검사를 끝낸 후 
        if(clear_users==0) 
        {
